...
from app.core.database import execute_one, execute_insert, execute_query
...
def collect_latest() -> str:
    ...
    with httpx.Client(timeout=10) as client:
        for _ in range(max_fetch):
            data = _fetch_round(client, settings.lotto_api_url, current)
            if not data:
                break
...
def collect_range(start: int, end: int) -> str:
    ...
    # 이미 존재하는 회차를 한 번에 조회 (N+1 방지)
    existing_rows = execute_query(
        "SELECT round FROM lotto_results WHERE round >= %s AND round <= %s",
        (start, end),
    )
    existing_rounds = {row["round"] for row in existing_rows}

    collected = 0
    skipped = len(existing_rounds)
    ...
    with httpx.Client(timeout=10) as client:
        for round_no in range(start, end + 1):
            ...
            data = _fetch_round(client, settings.lotto_api_url, round_no)
            if not data:
...
def _fetch_round(client: httpx.Client, base_url: str, round_no: int) -> dict | None:
...
def _insert_round(data: dict) -> bool:
    ...
    smok95 API 응답 예시:
    {
        ...
        "divisions": [{...}, {"prize": 47454864, "winners": 110}, ...],
    ...
    if len(set(numbers)) != 6:
        logger.warning(f"[Collector] 중복 번호 스킵: {round_no}회, numbers={numbers}")
        return False
